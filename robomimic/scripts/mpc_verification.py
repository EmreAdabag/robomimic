...
from pandadotpytorch.mpcpanda import (
...
def playback_dataset(args):
    ...
    if getattr(args, "use_mpc", False):
        assert not args.use_obs, "MPC playback only supports simulator-coupled mode (cannot be used with --use-obs)"
        urdf_path = "/home/jialeng/Desktop/robomimic/pandadotpytorch/robot_description/panda_with_gripper.urdf"
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        # Planning horizon and goal timesteps (single goal)
    ...
    for ind in range(len(demos)):
        ...
        # supply actions: preferentially via MPC, else dataset open-loop
        actions = None
        if getattr(args, "use_mpc", False) and (mpc_layer is not None):
            ...
            T_ep = obs_q.shape[0]
            # robosuite action dim is typically 8 (7 arm joints + 1 gripper)
            act_dim = max(n + 1, 8)
            actions = np.zeros((T_ep, act_dim), dtype=np.float32)

            for t in range(T_ep):
                q_t = torch.tensor(obs_q[t, :n], dtype=torch.get_default_dtype(), device=device)
                if obs_qdot is not None and obs_qdot.shape[1] >= n:
                    qd_t = torch.tensor(obs_qdot[t, :n], dtype=torch.get_default_dtype(), device=device)
                else:
                    qd_t = torch.zeros(n, dtype=torch.get_default_dtype(), device=device)
                x_init = torch.cat([q_t, qd_t]).unsqueeze(0)  # [1, 2n]

                # Use next-step joint position as a single goal (K=1)
                goal_q = torch.tensor(next_q[min(t, T_ep - 1), :n], dtype=torch.get_default_dtype(), device=device)
                ...
                actions[t, :n] = q_cmd.detach().cpu().numpy()
                # Simple gripper handling: keep open (1.0)
                actions[t, n] = 1.0

        elif args.use_actions:
            actions = f["data/{}/obs/robot0_joint_pos".format(ep)][()]
            # Append gripper channel (1.0 means open)
            if actions is not None:
                one_column = np.ones((actions.shape[0], 1), dtype=actions.dtype)
                actions = np.concatenate([actions, one_column], axis=1)
...
if __name__ == "__main__":
    ...
    # Use MPC to generate actions from dataset obs and next_obs
    parser.add_argument(
        "--use-mpc",
        action='store_true',
        help="use MPC-generated actions based on obs and next_obs (overrides --use-actions)",
    )
