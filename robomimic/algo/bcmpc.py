"""
Behavior Cloning with differentiable MPC (hybrid BC+MPC).

Minimal integration: the MLP predicts a noise vector that is subtracted
from baseline inputs (eef pose + default weights) to produce differentiable
MPC goals and weights. The MPC action is used for the action loss so that
gradients flow through MPC back into the MLP.
"""
...
@register_algo_factory_func("bcmpc")
def algo_config_to_class(algo_config):
    ...
    # For this integration, always use hybrid BC (MLP) + differentiable MPC.
    # Other BC variants are not enabled in this template.
    if rnn_enabled or transformer_enabled or gaussian_enabled or gmm_enabled or vae_enabled:
        raise NotImplementedError("Only MLP BC with MPC is supported in this template")
    algo_class, algo_kwargs = BCMPC, {}

    return algo_class, algo_kwargs


class BCMPC(PolicyAlgo):
    """Behavior Cloning with differentiable MPC over actions."""
    def _create_networks(self):
        self.nets = nn.ModuleDict()

        # MLP outputs 11-dim noise: [d_pos(3), d_quat(4), d_weights(4)]
        self._mpc_param_dim = 11
        self.nets["policy"] = PolicyNets.ActorNetwork(
            obs_shapes=self.obs_shapes,
            goal_shapes=self.goal_shapes,
            ac_dim=self._mpc_param_dim,
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        with TorchUtils.maybe_no_grad(no_grad=validate):
            info = super(BCMPC, self).train_on_batch(batch, epoch, validate=validate)
    ...
    def _forward_training(self, batch):
        ...
        obs = batch["obs"]
        assert "robot0_eef_pos" in obs, "obs must include 'robot0_eef_pos'"
        assert "robot0_eef_quat" in obs, "obs must include 'robot0_eef_quat' (xyzw)"

        mlp_out = self.nets["policy"](obs_dict=obs, goal_dict=batch["goal_obs"])  # [B, 11]
        assert mlp_out.shape[-1] == self._mpc_param_dim
        ...
        assert "robot0_joint_vel" in obs or "robot0_joint_qvel" in obs, "obs must include joint velocities for x_init (dq)"
        q = obs["robot0_joint_pos"].float()
        dq_key = "robot0_joint_vel" if "robot0_joint_vel" in obs else "robot0_joint_qvel"
        dq = obs[dq_key].float()
        assert q.ndimension() == 2 and dq.ndimension() == 2 and q.shape == dq.shape
        x_init = torch.cat([q, dq], dim=-1)

        B = x_init.size(0)
        K = 1
        goal_positions = goal_pos.view(B, K, 3)
        goal_quaternions = goal_quat.view(B, K, 4)

        x_traj, u_traj, _ = self.mpc_layer(
            x_init=x_init,
    ...
    def log_info(self, info):
        ...
        """
        log = super(BCMPC, self).log_info(info)
    ...
    def get_action(self, obs_dict, goal_dict=None):
        ...
        assert "robot0_eef_pos" in obs_dict
        assert "robot0_eef_quat" in obs_dict
        mlp_out = self.nets["policy"](obs_dict, goal_dict=goal_dict)
        assert mlp_out.shape[-1] == self._mpc_param_dim
        ...
        q = obs_dict["robot0_joint_pos"].float()
        dq_key = "robot0_joint_vel" if "robot0_joint_vel" in obs_dict else "robot0_joint_qvel"
        dq = obs_dict[dq_key].float()
        assert q.ndimension() == 2 and dq.ndimension() == 2 and q.shape == dq.shape
        x_init = torch.cat([q, dq], dim=-1)

        B = x_init.size(0)
        goal_positions = goal_pos.view(B, 1, 3)
        goal_quaternions = goal_quat.view(B, 1, 4)
